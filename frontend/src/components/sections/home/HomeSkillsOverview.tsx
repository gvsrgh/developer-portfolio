        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
            {skillCategories.map((category, index) => (
              ...
              >
                <div 
                  className="bg-gray-50 dark:bg-gray-700 rounded-xl p-6 h-full hover:shadow-2xl transition-all duration-100 relative"
                  style={{
                ...
                >
                  {/* Icon */}
                  <div className={`w-12 h-12 rounded-lg bg-gradient-to-r ${category.color} p-3 mb-4 group-hover:scale-110 transition-transform duration-100`}>
                    <category.icon className="w-full h-full text-white" />
                  </div>

                  {/* Content */}
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors duration-100">
                    {category.title}
                  </h3>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(expandedCategories.has(category.title) ? category.skills : category.skills.slice(0, 3)).map((skill) => (
                      <span
                        key={skill}
                        className="px-2 py-1 bg-white dark:bg-gray-600 text-xs font-medium rounded-md text-gray-700 dark:text-gray-200 hover:bg-blue-50 dark:hover:bg-blue-900 transition-colors duration-100"
                      >
                        {skill}
                      </span>
                    ))}
                    {category.skills.length > 3 && !expandedCategories.has(category.title) && (
                      <button
                        ...
                        className="px-2 py-1 bg-gray-200 dark:bg-gray-500 text-xs font-medium rounded-md text-gray-600 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-400 transition-colors cursor-pointer hover:scale-105 transform duration-100"
                      >
          ...
          <motion.div
            ...
            className="bg-gradient-to-r from-blue-500/10 to-purple-600/10 rounded-2xl p-8 mb-12 hover:shadow-xl transition-all duration-100"
            style={{
              filter: 'drop-shadow(0 0 0 transparent)',
              transition: 'all 0.1s ease-in-out, filter 0.1s ease-in-out',
            }}
